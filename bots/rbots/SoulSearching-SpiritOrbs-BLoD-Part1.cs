...
public class Script
{
    ...
    int FarmLoop = 0;
    int SavedState = 0;
    public readonly int[] QuestList = { 939, 2082, 2083 };
    ...
    public void ScriptMain(ScriptInterface bot)
    {
        ...
        bot.SendPacket("%xt%zm%getQuests%40245%4703%4704%5755%5756%6743%6750%7073%");

        while (!bot.ShouldExit())
        {
            while (!bot.Inventory.Contains("Spirit Orb", 10500))
            {
                if (!bot.Quests.IsAvailable(939))
                {
                    Console.WriteLine("Please Complete /battleunderc to run this bot.");
                    StopBot("battleunderc");
                }
                if (!bot.Quests.IsAvailable(2082))
                {
                    Console.WriteLine("Please Unlock Essential Essences from Artix in /necropolis.");
                    StopBot("necropolis");
                }
                foreach (var Quest in QuestList)
                {
                    while (!bot.Quests.IsInProgress(Quest)) bot.Quests.EnsureAccept(Quest);
                    while (bot.Quests.CanComplete(Quest)) SafeQuestComplete(Quest);
                }
                while (bot.Inventory.Contains("Undead Essence", 25)) SafeQuestComplete(2082);
                TempItemFarm("Bone Terror Soul", 1, "battleundera", "r7", "Right", 939);
                TempItemFarm("Undead Champion Soul", 1, "battleunderb", "r3", "Right", 939);
                TempItemFarm("Jellyfish Soul", 1, "battleunderc", "r5", "Left", 939);
