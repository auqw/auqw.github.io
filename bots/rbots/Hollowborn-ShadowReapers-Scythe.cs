...
public class BluuTemplate
{
	...
	public string[] HollowbornComponents = {
		"Hollowborn Reaper's Kama",
		"Hollowborn Reaper's Kamas",
		"Hollowborn Reaper's Daggers"
	};
	...
	public void ScriptMain(ScriptInterface bot)
	{
		...
		while (!bot.ShouldExit())
		{
			...
			while (bot.Player.Level < 80)
			{
				FarmIceStormArena();
			}
			while (bot.Player.GetFactionRank("Hollowborn") < 9)
			{
				FarmShadowRealm();
			}
			foreach (var Component in HollowbornComponents)
			{
				if (!bot.Inventory.Contains(Component, 1))
				{
					FarmComponentReqs(250, 3030, 5, 1, 1);
					SafePurchase(Component, 1, "shadowrealm", 1889);
				}
			}
			if (!bot.Inventory.Contains("Hollowborn Reaper's Minion", 1))
			{
				FarmComponentReqs(150, 2020, 2, 0, 0);
			...
			StopBot("Obtained all requirements for the Hollowborn ShadowReaper's Scythe.", "shadowrealm", "r10", "Bottom");
		}
	...
	maintainFarmLoop:
		FarmLoop += 1;
		if (bot.Map.Name != "icestormarena") SafeMapJoin("icestormarena", "r3c", "Top");
	...
	public void FarmComponentReqs(int HollowSoul, int BoneDust, int DeathsOversight, int GlitchScythe, int UnmouldedEssence)
	{
		while (!bot.Inventory.Contains("Hollow Soul", HollowSoul)) FarmShadowRealm();
		InvItemFarm("Bone Dust", BoneDust, "battleunderb", "Enter", "Spawn");
		InvItemFarm("Death's Oversight", DeathsOversight, "shadowattack", "Boss", "Left");
		InvItemFarm("Incarnation of Glitches Scythe", GlitchScythe, "cathedral", "r14", "Left");
		if (UnmouldedEssence > 0)
		{
			while (bot.Player.Gold < 50000000) FarmIceStormArena();
