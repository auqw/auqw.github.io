...
public class BluuPurpleTemplate
{
	...
	public string[] Combined = Ver1Items.Concat(Ver2Items).Concat(Ver3Items).ToArray();
	public string[] EquippedItems = { };
    public readonly int[] QuestList = { 929,2163,2178,2082 };
	//-----------EDIT ABOVE-------------//
	...
	public void ScriptMain(ScriptInterface bot)
	{
		...
		while (!bot.ShouldExit())
		{
			...
			if (!bot.Quests.IsAvailable(939))
                {
                    MessageBox.Show($"Please Complete /battleunderc to run this bot.");
                    StopBot();
                }

            if (!bot.Quests.IsAvailable(2082))
                {
                    MessageBox.Show($"Please Unlock Essential Essences from Artix in /necropolis.");
                    StopBot();
                }
    ...
    {    
        ...
        FormatLog(Title: true, Text: "Undead Essence Farming"); 
    ...
    public void BroadSword(int Quantity)
    {
        ...
        FormatLog(Title: true, Text: "Spirit Orb Farming Ver1"); 

		int i = 1;   
        while (!bot.Inventory.Contains("Spirit Orb", Quantity))
        {   
        //Blinding Light Fragments x10 
        ItemFarm("Blinding Light Fragments", 10, false, false, 2178, "Skeleton Warrior", "battleunderb", "Enter", "Spawn");
    ...
    public void UltiKit(int Quantity) 
    {
        ...
        FormatLog(Title: true, Text: "Spirit Orb Farming Ver2"); 

		int i = 1;   
        while (!bot.Inventory.Contains("Spirit Orb", Quantity))
        {   
        ItemFarm("Great Ornate Warhammer", 1, false, true, 2163, "Earth Elemental", "gilead");
        ItemFarm("No. 1337 Blade Oil", 1, true, true, 2163, "Kitsune", "kitsune");
        ItemFarm("Greenguard Dragon Hide", 3, true, true, 2163, "Greenguard Dragon", "dragonchallenge");
        ItemFarm("Gold Brush", 1, true, true, 2163, "Sphinx", "sandcastle");
        ItemFarm("Sharp Stone Sharpener", 1, true, true, 2163, "Rock Roc", "roc");
        ItemFarm("Blinding Lacquer Finish", 1, true, true, 2163, "Grand Inquisitor", "citadel");
        ItemFarm("Non-abrasive Power Powder", 1, true, true, 2163, "Protosartorium", "crashsite");
        ItemFarm("Suede Travel Case", 1, true, true, 2163, "Harpy", "djinn");
        SafeQuestComplete(2163);
        i++;
        }            
    }
    ...
    {          
        ...
        FormatLog(Title: true, Text: "Spirit Orb Farming Ver3");  

		int i = 1;   
        while (!bot.Inventory.Contains("Spirit Orb", Quantity))
        {   
            EssentialEssences(25);
