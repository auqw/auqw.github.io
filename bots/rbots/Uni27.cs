...
public class Uni27 //🥔
{
    ...
    public string SoloClass = "Void Highlord"; //<-----Edit to your preference - SafeEquip(SoloClass);  https://github.com/BrenoHenrike/RBot-Scripts - run brenos "5Wolfwing(Darkovia)" in the chaoslords folder (use the guide on the site to configure your folder correctly)
    public string FarmClass = "Legion Revenant"; //<-----Edit to your preference - SafeEquip(FarmClass);
	public static string[] Larvae = {"Mana Energy For Nulgath", "Unidentified 10", "Unidentified 13", "Tainted Gem", "Dark Crystal Shard", "Diamond of Nulgath", "Voucher of Nulgath", "Voucher of Nulgath (non-mem)", "Totem of Nulgath", "Gem of Nulgath", "Blood Gem of the Archfiend"};
	public static string[] Uni26 = { "War-Torn Memorabilia", "Unidentified 19", "Tainted Gem", "Dark Crystal Shard", "Diamond of Nulgath", "Voucher of Nulgath (non-mem)", "Unidentified 26", "Unidentified 27",  };
	public string[] combined = Larvae.Concat(Uni26).ToArray();
	...
	public void ScriptMain(ScriptInterface bot)
	{
		...
		while (!bot.ShouldExit())
		{
			...
			while(!bot.Inventory.Contains("Unidentified 27") && !bot.Bank.Contains("Unidentified 27"))
			{
				while(!bot.Inventory.Contains("Unidentified 26") && !bot.Bank.Contains("Unidentified 26"))
				{
					...
					{
						while(!bot.Inventory.Contains("War-Torn Memorabilia", 1))
						{				
				...
				while(!bot.Inventory.ContainsTempItem("Dark Makai Sigil"))
				{	
					SkillList(FarmClass, SkillOrder);
					SafeEquip(FarmClass);					
					bot.Quests.EnsureAccept(584);			
					ItemFarm("Dark Makai Sigil", 1, true, false, 0 ,"Dark Makai", "tercessuinotlim", "m2", "Center");
				}

				while(!bot.Inventory.Contains("Tainted Gem", 5)){ TaintedGemFarm(bot, "Tainted Gem", 5); }
				
			SafeQuestComplete(584);
			bot.Wait.ForDrop("Unidentified 27");
			bot.Player.Pickup("Unidentified 27");
			}
		StopBot($"Unidentified 27/Ordinary Cape (Obtained)");
		}
	}
		...
		if(bot.Inventory.Contains(item, quantity))
			return;
			SkillList(SoloClass, SkillOrder);
			SafeEquip(SoloClass);
			bot.Log("LarvaeStart");
			...
			while(!bot.Inventory.Contains(item, quantity)){SafeQuestComplete(2566);}
		}

		public void TaintedGemFarm(ScriptInterface bot, string item, int quantity)
		{
		bot.Bank.ToInventory(item);
		if(bot.Inventory.Contains(item, quantity))
			return;
