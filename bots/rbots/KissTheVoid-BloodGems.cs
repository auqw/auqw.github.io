...
public class BluuTemplate
{
	...
	public void ScriptMain(ScriptInterface bot)
	{
		...
		while (!bot.ShouldExit())
		{
			...
			while (!bot.Inventory.Contains("Blood Gem of the Archfiend", 100))
            {
				if (bot.Inventory.Contains("Unidentified 10", 50) && bot.Player.Gold > 900000) Emporium();
				if (bot.Inventory.Contains("Voucher of Nulgath (non-mem)") && bot.Inventory.Contains("Essence of Nulgath", 60))
				{
					if (!bot.Inventory.Contains("Totem of Nulgath", 100) || !bot.Inventory.Contains("Gem of Nulgath", 300)) VoucherItem();
				}
				InvItemFarm("Tendurrr The Assistant", 1, "tercessuinotlim", "m2", "Left", 3743);
				InvItemFarm("Fragment of Chaos", 80, "blindingsnow", "r17", "Left", 3743);
				TempItemFarm("Broken Betrayal Blade", 8, "underwworld", "r14", "Left", 3743);
				SafeQuestComplete(3743);
            }
		}
	...
	*/
    ...
    {
		SafePurchase("Receipt of Swindle", 3, "tercessuinotlim", 1951);
		if (!bot.Inventory.Contains("Receipt of Swindle", 3)) { }
		bot.Shops.BuyItem("Blood Gem of the Archfiend");
	}


	public void VoucherItem()
	{
		if (!bot.Inventory.Contains("Totem of Nulgath", 100)) SafeQuestComplete(4778, 5357); 
		else if (!bot.Inventory.Contains("Gem of Nulgath", 300)) SafeQuestComplete(4778, 6136);
		else { }
	}
