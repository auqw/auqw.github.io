...
public class BluuTemplate
{
	...
	public void ScriptMain(ScriptInterface bot)
	{
		...
		while (!bot.ShouldExit())
		{
			...
			while (!bot.Inventory.Contains("Blood Gem of the Archfiend", BloodGemOfTheArchfiendQuantity) || 
				!bot.Inventory.Contains("Diamond of Nulgath", DiamondOfNulgathQuantity) || 
				!bot.Inventory.Contains("Dark Crystal Shard", DarkCrystalShardQuantity) || 
				!bot.Inventory.Contains("Totem of Nulgath", TotemOfNulgathQuantity) || 
				!bot.Inventory.Contains("Unidentified 13", Unidentified13Quantity) || 
				!bot.Inventory.Contains("Gem of Nulgath", GemOfNulgathQuantity) || 
				!bot.Inventory.Contains("Fiend Token", FiendTokenQuantity))
			{
				if (bot.Inventory.Contains("Voucher of Nulgath (non-mem)") && bot.Inventory.Contains("Essence of Nulgath", 60))
                {
					if (!bot.Inventory.Contains("Totem of Nulgath", TotemOfNulgathQuantity) || !bot.Inventory.Contains("Gem of Nulgath", GemOfNulgathQuantity)) VoucherItem();
				}
				if (bot.Inventory.Contains("Bounty Hunter's Drone Pet")) NWNO(6183);
				if (bot.Inventory.Contains("Nulgath's Birthday Gift")) NWNO(6697);
			}
			while (!bot.Inventory.Contains("Tainted Gem", TaintedGemQuantity)) SwindleBulk();
			StopBot("All Requested Nulgath Reagents successfully obtained.", "tercessuinotlim");
		}
	...
	*/
    ...
    {
		while (!bot.Inventory.Contains("Tainted Gem", TaintedGemQuantity))
        {
    ...
    {
		if (!bot.Inventory.Contains("Gem of Nulgath", GemOfNulgathQuantity)) SafeQuestComplete(4778, 6136);
		else if (!bot.Inventory.Contains("Totem of Nulgath", TotemOfNulgathQuantity)) SafeQuestComplete(4778, 5357);
		else { }
	}
